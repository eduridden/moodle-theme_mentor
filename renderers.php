...
class theme_mentor_core_renderer extends core_renderer {
    private function footermod_aboutus(){
        $logourl = get_config('theme_mentor', 'footermod_aboutus_whitelogo');
        $text = get_config('theme_mentor', 'footermod_aboutus_text');
    ...
    private function footermod_links(){
        $links = json_decode(get_config('theme_mentor', 'footermod_links'));
    ...
    private function footermod_contactinfo(){
        $address = get_config('theme_mentor', 'footermod_contact_address');
        $city = get_config('theme_mentor', 'footermod_contact_city');
        $phone = get_config('theme_mentor', 'footermod_contact_phone');
        $mail = get_config('theme_mentor', 'footermod_contact_mail');
    ...
    private function footermod_image(){
        $title = get_config('theme_mentor', 'footermod_image_title');
        $src = get_config('theme_mentor', 'footermod_image_url');
    ...
    protected function footermod($modulearea){
        $module = get_config("theme_mentor","footer".$modulearea);
        if(trim($module)!=""){
    ...
    protected function linkbox($CFG,$sidebar){
        $linkboxitems = json_decode(get_config('theme_mentor', 'linkboxdata'));
        $content = '';
        ...
        for($x=1;$x<=sizeof($linkboxitems);$x++){
            ...
            $content .= '<div class="four columns linkbox '.$align.'">
                            ...
                            <a href="'.$linkboxitems[$x-1]->link.'"><div class="readmore">Read More</div></a>
                        </div>';
        }
        return $content;
    }
    protected function mycourses($CFG,$sidebar){
        ...
        for($x=1;$x<=sizeof($courselist);$x++){
            ...
            $url = $CFG->wwwroot."/theme/mentor/pix/coursenoimage.jpg";
    ...
    protected function menu(){
        $menuitems = json_decode(get_config('theme_mentor', 'menudata'));
        $content = '<nav id="menu">';
    ...
    protected function socialicons($area){
        $hassocialicons = get_config('theme_mentor', $area.'socialicon');
    ...
    protected function slider($pagelayout){
        if($pagelayout == 'frontpage')
            $slidertype = get_config('theme_mentor', 'slidermode');
        else
            $slidertype = 'banner';
        
        $sliderpattern = "slider".get_config('theme_mentor', 'sliderpattern').get_config('theme_mentor', 'themecolor');
        $slideritems = json_decode(get_config('theme_mentor', 'slideshowdata'));
    ...
    protected function render_navigation_node(navigation_node $item) {
        ...
        if ($item->icon instanceof renderable && !$item->hideicon) {
            if(trim($content) == 'mentor')
                $item->icon->pix = 'g/mentor_'.get_config('theme_mentor','themecolor');
